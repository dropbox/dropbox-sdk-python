...
parser = argparse.ArgumentParser(
    description='Upload a folder of files to Dropbox in a performant manner.')
parser.add_argument('--local_path', required=True,
    help='Local path of folder of files to upload (non-recursive).')
parser.add_argument('--remote_path', required=True,
    help='Remote path of folder in Dropbox to upload to.')
parser.add_argument('--access_token',
    help='Access token to use to perform the API calls.')
parser.add_argument('--refresh_token',
    help='Refresh token to use to retrieve access tokens.')
parser.add_argument('--app_key',
    help='App key to use to retrieve access tokens. Required when using a refresh token.')
parser.add_argument('--app_secret',
    help='App secret to use to retrieve access tokens.')

# These three values can be tuned for better (or worse) overall performance,
# based on the scenario and environment:
parser.add_argument('--chunk_size', default=24 * MB,
    help='The amount of data, in bytes, to send per upload request.')
parser.add_argument('--batch_thread_count', default=20,
    help='How many threads to use per batch of upload sessions.')
parser.add_argument('--concurrent_thread_count', default=10,
    help='How many threads to use per upload session.')
        ...
        for index, file in enumerate(files):
            futures.append(
                batch_executor.submit(
                    ...
                    client, start_batch_result.session_ids[index], file, args
                )
            )
