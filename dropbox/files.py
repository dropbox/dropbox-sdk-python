...
class Metadata(object):
    """
    Metadata for a file or folder. You should never see a bare :class:`Metadata`
    instance; you will always receive an instance of :class:`FileMetadata` or
...
class FileMetadata(Metadata):
    ...
    def __repr__(self):
        return 'FileMetadata(name={!r}, path_lower={!r}, client_modified={!r}, server_modified={!r}, rev={!r}, size={!r})'.format(
            self._name_value,
            self._path_lower_value,
...
class FolderMetadata(Metadata):
    ...
    def __repr__(self):
        return 'FolderMetadata(name={!r}, path_lower={!r})'.format(
            self._name_value,
            self._path_lower_value,
        )
...
class GetMetadataError(object):
    ...
    :ivar not_found: File is not found at the specified path.
...
class GetMetadataArg(object):
    ...
    :ivar path: The path of the file or folder on Dropbox. Must not be the root.
    """
...
class ListFolderArg(object):
    ...
    :ivar path: The path to the folder you want to see the contents of. May be
        the root (i.e. empty).
    """
    ...
    def path(self):
        """
        The path to the folder you want to see the contents of. May be the root
        (i.e. empty).
    ...
    def __repr__(self):
        return 'ListFolderArg(path={!r})'.format(
            self._path_value,
        )
...
class FolderPathError(object):
    """
    Base class for errors returned by list_folder.

    :ivar not_found: There is no file or folder at the given path.
    :ivar not_folder: Entry at path is not a folder.
    """
    ...
    def __repr__(self):
        return 'FolderPathError(%r)' % self._tag

class ListFolderError(FolderPathError):
    """
...
class ListFolderContinueError(ListFolderError):
    """
...
class NoFileReason(object):
    """
    Errors from download when the file cannot be downloaded.

    :ivar not_found: The path is not found.
    :ivar is_folder: The path refers to a folder. Only files can be downloaded.
    """
    ...
    def __repr__(self):
        return 'NoFileReason(%r)' % self._tag

class NoFile(object):
    """
    Error structure for download.

    :ivar reason: The path could not be downloaded. The value gives the reason.
    """
    ...
    def __repr__(self):
        return 'NoFile(reason={!r})'.format(
            self._reason_value,
        )

class RestrictedReason(object):
    """
    Errors for download when download is forbidden.

    :ivar dmca: The download is forbidden because of a DMCA (U.S. Digital
        Millenium Copyright Act) takedown request.
    :ivar other: The download is forbidden for some other reason.
    """
    ...
    def is_dmca(self):
        return self._tag == 'dmca'

    def is_other(self):
        return self._tag == 'other'

    def __repr__(self):
        return 'RestrictedReason(%r)' % self._tag

class Restricted(object):
    """
    Error structure for download.

    :ivar reason: The download is forbidden. The value gives the reason.
    """
    ...
    def __repr__(self):
        return 'Restricted(reason={!r})'.format(
            self._reason_value,
        )

class DisallowedReason(object):
    """
    Errors for download when download is disallowed or forbidden.

    :ivar permission: The requesting user has no permission to access the file.
    :ivar Restricted restricted: The download is forbidden; see the value for
        the reason.
    """
    ...
    @classmethod
    def restricted(cls, val):
        return cls('restricted', val)

    def is_permission(self):
        return self._tag == 'permission'

    def is_restricted(self):
        return self._tag == 'restricted'

    def get_restricted(self):
        if not self.is_restricted():
            raise AttributeError("tag 'restricted' not set")
        return self._value

    def __repr__(self):
        return 'DisallowedReason(%r)' % self._tag

class Disallowed(object):
    """
    Error structure for download.

    :ivar reason: The reason why the download is disallowed.
    """
    ...
    def __repr__(self):
        return 'Disallowed(reason={!r})'.format(
            self._reason_value,
        )
...
class DownloadError(object):
    ...
    :ivar Disallowed disallowed: The download is disallowed or forbidden; see
        the value for the reason.
    :ivar NoFile no_file: The path cannot be downloaded.
    :ivar other: An unspecified error.
    """
    ...
    @classmethod
    def disallowed(cls, val):
        return cls('disallowed', val)

    @classmethod
    def no_file(cls, val):
        return cls('no_file', val)
...
class CommitConflictError(object):
    """
    Errors related to commit conflicts.

    :ivar file: A file already exists at this path.
    :ivar folder: A folder already exists at this path.
    :ivar autorename_failed: File could not be automatically renamed.
    :ivar other: An unspecified error.
    """
    ...
    def __repr__(self):
        return 'CommitConflictError(%r)' % self._tag

class CommitError(object):
    """
    Errors from upload.

    :ivar CommitConflictError conflict: A conflict prevented the commit. See the
        value for the reason.
    :ivar no_write_permission: User does not have permission to write in the
        folder. An example of this is if the folder is a read-only shared
        folder.
    :ivar insufficient_quota: User does not have sufficient space quota to save
        the file.
    :ivar other: An unspecified error.
    """
    ...
    def __repr__(self):
        return 'CommitError(%r)' % self._tag

class UploadCommitError(object):
    """
    Error structure for commit errors.

    :ivar reason: The reason why the commit failed.
    ...
    def __repr__(self):
        return 'UploadCommitError(reason={!r}, upload_session_id={!r})'.format(
            self._reason_value,
...
class UploadError(object):
    ...
    :ivar UploadCommitError commit_failed: The upload failed; the value explains
        the reason.
    :ivar other: An unspecified error.
    """
    ...
    @classmethod
    def commit_failed(cls, val):
        return cls('commit_failed', val)

    def is_commit_failed(self):
        return self._tag == 'commit_failed'

    def is_other(self):
        return self._tag == 'other'

    def get_commit_failed(self):
        if not self.is_commit_failed():
            raise AttributeError("tag 'commit_failed' not set")
        return self._value
...
class UploadSessionFinishError(object):
    ...
    :ivar CommitError commit_failed: The commit failed; the value explains the
        reason.
    :ivar other: An unspecified error.
    """
...
class SearchError(FolderPathError):
    """
    Errors for search.
...
class PathError(object):
    """
    Path validation errors.

    :ivar root_disallowed: The path should not be empty.
    :ivar str malformed_path: The path is invalid; the message gives more info.
    :ivar not_found: The path does not exist.
    :ivar file_disallowed: The path should refer to a folder.
    :ivar folder_disallowed: The path should refer to a file.
    :ivar conflict_file: A file already exists there.
    :ivar conflict_folder: A folder already exists there.
    :ivar conflict_file_ancestor: An ancestor is a file.
    :ivar read_only: No write permission.
    :ivar shared_folder_disallowed: The path should not refer to a shared
        folder.
    :ivar autorename_filed: Automatic rename would result in a malformed path
        (e.g. too long).
    :ivar str misc: Miscellaneous error (see error message).
    :ivar other: An unspecified error.
    """
    ...
    _catch_all = 'other'
    # Attribute is overwritten below the class definition
    root_disallowed = None
    # Attribute is overwritten below the class definition
    not_found = None
    # Attribute is overwritten below the class definition
    file_disallowed = None
    # Attribute is overwritten below the class definition
    folder_disallowed = None
    # Attribute is overwritten below the class definition
    conflict_file = None
    # Attribute is overwritten below the class definition
    conflict_folder = None
    # Attribute is overwritten below the class definition
    conflict_file_ancestor = None
    # Attribute is overwritten below the class definition
    read_only = None
    # Attribute is overwritten below the class definition
    shared_folder_disallowed = None
    # Attribute is overwritten below the class definition
    autorename_filed = None
    # Attribute is overwritten below the class definition
    other = None
    ...
    def is_root_disallowed(self):
        return self._tag == 'root_disallowed'
    ...
    def is_file_disallowed(self):
        return self._tag == 'file_disallowed'

    def is_folder_disallowed(self):
        return self._tag == 'folder_disallowed'

    def is_conflict_file(self):
        return self._tag == 'conflict_file'

    def is_conflict_folder(self):
        return self._tag == 'conflict_folder'

    def is_conflict_file_ancestor(self):
        return self._tag == 'conflict_file_ancestor'

    def is_read_only(self):
        return self._tag == 'read_only'

    def is_shared_folder_disallowed(self):
        return self._tag == 'shared_folder_disallowed'

    def is_autorename_filed(self):
        return self._tag == 'autorename_filed'

    def is_misc(self):
        return self._tag == 'misc'
    ...
    def get_misc(self):
        if not self.is_misc():
            raise AttributeError("tag 'misc' not set")
        return self._value

    def __repr__(self):
        return 'PathError(%r)' % self._tag
...
class RelocationError(object):
    ...
    :ivar PathError from_error: The operation is disallowed due to a problem
        with the source path.
    :ivar PathError to_error: The operation is disallowed due to a problem with
        the destination path.
    :ivar too_many_files: The operation would involve more than 10,000 files and
        folders.
    :ivar quota_error: Quota exceeded.
    :ivar other: An unspecified error.
    """
    ...
    @classmethod
    def from_error(cls, val):
        return cls('from_error', val)

    @classmethod
    def to_error(cls, val):
        return cls('to_error', val)

    def is_from_error(self):
        return self._tag == 'from_error'

    def is_to_error(self):
        return self._tag == 'to_error'

    def is_too_many_files(self):
        return self._tag == 'too_many_files'

    def is_quota_error(self):
        return self._tag == 'quota_error'

    def is_other(self):
        return self._tag == 'other'

    def get_from_error(self):
        if not self.is_from_error():
            raise AttributeError("tag 'from_error' not set")
        return self._value

    def get_to_error(self):
        if not self.is_to_error():
            raise AttributeError("tag 'to_error' not set")
        return self._value
...
class ThumbnailError(object):
    ...
    :ivar DownloadError download_error: An error occurs when downloading
    ...
    @classmethod
    def download_error(cls, val):
        return cls('download_error', val)

    def is_download_error(self):
        return self._tag == 'download_error'
    ...
    def get_download_error(self):
        if not self.is_download_error():
            raise AttributeError("tag 'download_error' not set")
        return self._value
...
GetMetadataError._not_found_validator = bv.Void()
GetMetadataError._other_validator = bv.Void()
GetMetadataError._tagmap = {
    'not_found': GetMetadataError._not_found_validator,
    'other': GetMetadataError._other_validator,
}

GetMetadataError.not_found = GetMetadataError('not_found')
GetMetadataError.other = GetMetadataError('other')

GetMetadataArg._path_validator = bv.String()
GetMetadataArg._all_field_names_ = set(['path'])
GetMetadataArg._all_fields_ = [('path', GetMetadataArg._path_validator)]

ListFolderArg._path_validator = bv.String()
ListFolderArg._all_field_names_ = set(['path'])
ListFolderArg._all_fields_ = [('path', ListFolderArg._path_validator)]
...
FolderPathError._not_found_validator = bv.Void()
FolderPathError._not_folder_validator = bv.Void()
FolderPathError._tagmap = {
    'not_found': FolderPathError._not_found_validator,
    'not_folder': FolderPathError._not_folder_validator,
}

FolderPathError.not_found = FolderPathError('not_found')
FolderPathError.not_folder = FolderPathError('not_folder')

ListFolderError._other_validator = bv.Void()
ListFolderError._tagmap = {
    'other': ListFolderError._other_validator,
}
ListFolderError._tagmap.update(FolderPathError._tagmap)
...
ListFolderContinueError._tagmap.update(ListFolderError._tagmap)

ListFolderContinueError.reset = ListFolderContinueError('reset')

NoFileReason._not_found_validator = bv.Void()
NoFileReason._is_folder_validator = bv.Void()
NoFileReason._tagmap = {
    'not_found': NoFileReason._not_found_validator,
    'is_folder': NoFileReason._is_folder_validator,
}

NoFileReason.not_found = NoFileReason('not_found')
NoFileReason.is_folder = NoFileReason('is_folder')

NoFile._reason_validator = bv.Union(NoFileReason)
NoFile._all_field_names_ = set(['reason'])
NoFile._all_fields_ = [('reason', NoFile._reason_validator)]

RestrictedReason._dmca_validator = bv.Void()
RestrictedReason._other_validator = bv.Void()
RestrictedReason._tagmap = {
    'dmca': RestrictedReason._dmca_validator,
    'other': RestrictedReason._other_validator,
}

RestrictedReason.dmca = RestrictedReason('dmca')
RestrictedReason.other = RestrictedReason('other')

Restricted._reason_validator = bv.Union(RestrictedReason)
Restricted._all_field_names_ = set(['reason'])
Restricted._all_fields_ = [('reason', Restricted._reason_validator)]

DisallowedReason._permission_validator = bv.Void()
DisallowedReason._restricted_validator = bv.Struct(Restricted)
DisallowedReason._tagmap = {
    'permission': DisallowedReason._permission_validator,
    'restricted': DisallowedReason._restricted_validator,
}

DisallowedReason.permission = DisallowedReason('permission')

Disallowed._reason_validator = bv.Union(DisallowedReason)
Disallowed._all_field_names_ = set(['reason'])
Disallowed._all_fields_ = [('reason', Disallowed._reason_validator)]

DownloadError._disallowed_validator = bv.Struct(Disallowed)
DownloadError._no_file_validator = bv.Struct(NoFile)
DownloadError._other_validator = bv.Void()
DownloadError._tagmap = {
    'disallowed': DownloadError._disallowed_validator,
    'no_file': DownloadError._no_file_validator,
    'other': DownloadError._other_validator,
}

DownloadError.other = DownloadError('other')

DownloadArg._path_validator = bv.String()
...
CommitConflictError._file_validator = bv.Void()
CommitConflictError._folder_validator = bv.Void()
CommitConflictError._autorename_failed_validator = bv.Void()
CommitConflictError._other_validator = bv.Void()
CommitConflictError._tagmap = {
    'file': CommitConflictError._file_validator,
    'folder': CommitConflictError._folder_validator,
    'autorename_failed': CommitConflictError._autorename_failed_validator,
    'other': CommitConflictError._other_validator,
}

CommitConflictError.file = CommitConflictError('file')
CommitConflictError.folder = CommitConflictError('folder')
CommitConflictError.autorename_failed = CommitConflictError('autorename_failed')
CommitConflictError.other = CommitConflictError('other')

CommitError._conflict_validator = bv.Union(CommitConflictError)
CommitError._no_write_permission_validator = bv.Void()
CommitError._insufficient_quota_validator = bv.Void()
CommitError._other_validator = bv.Void()
CommitError._tagmap = {
    'conflict': CommitError._conflict_validator,
    'no_write_permission': CommitError._no_write_permission_validator,
    'insufficient_quota': CommitError._insufficient_quota_validator,
    'other': CommitError._other_validator,
}

CommitError.no_write_permission = CommitError('no_write_permission')
CommitError.insufficient_quota = CommitError('insufficient_quota')
CommitError.other = CommitError('other')

UploadCommitError._reason_validator = bv.Union(CommitError)
UploadCommitError._upload_session_id_validator = bv.String()
UploadCommitError._all_field_names_ = set([
    'reason',
    'upload_session_id',
])
UploadCommitError._all_fields_ = [
    ('reason', UploadCommitError._reason_validator),
    ('upload_session_id', UploadCommitError._upload_session_id_validator),
]
...
UploadError._commit_failed_validator = bv.Struct(UploadCommitError)
UploadError._other_validator = bv.Void()
UploadError._tagmap = {
    'commit_failed': UploadError._commit_failed_validator,
...
UploadSessionFinishError._commit_failed_validator = bv.Union(CommitError)
...
UploadSessionFinishError._tagmap = {
    ...
    'commit_failed': UploadSessionFinishError._commit_failed_validator,
...
CommitInfo._path_validator = bv.String()
...
SearchQuery._path_validator = bv.String()
...
SearchError._tagmap.update(FolderPathError._tagmap)

SearchError.other = SearchError('other')

PathError._root_disallowed_validator = bv.Void()
PathError._malformed_path_validator = bv.String()
PathError._not_found_validator = bv.Void()
PathError._file_disallowed_validator = bv.Void()
PathError._folder_disallowed_validator = bv.Void()
PathError._conflict_file_validator = bv.Void()
PathError._conflict_folder_validator = bv.Void()
PathError._conflict_file_ancestor_validator = bv.Void()
PathError._read_only_validator = bv.Void()
PathError._shared_folder_disallowed_validator = bv.Void()
PathError._autorename_filed_validator = bv.Void()
PathError._misc_validator = bv.String()
PathError._other_validator = bv.Void()
PathError._tagmap = {
    'root_disallowed': PathError._root_disallowed_validator,
    'malformed_path': PathError._malformed_path_validator,
    'not_found': PathError._not_found_validator,
    'file_disallowed': PathError._file_disallowed_validator,
    'folder_disallowed': PathError._folder_disallowed_validator,
    'conflict_file': PathError._conflict_file_validator,
    'conflict_folder': PathError._conflict_folder_validator,
    'conflict_file_ancestor': PathError._conflict_file_ancestor_validator,
    'read_only': PathError._read_only_validator,
    'shared_folder_disallowed': PathError._shared_folder_disallowed_validator,
    'autorename_filed': PathError._autorename_filed_validator,
    'misc': PathError._misc_validator,
    'other': PathError._other_validator,
}

PathError.root_disallowed = PathError('root_disallowed')
PathError.not_found = PathError('not_found')
PathError.file_disallowed = PathError('file_disallowed')
PathError.folder_disallowed = PathError('folder_disallowed')
PathError.conflict_file = PathError('conflict_file')
PathError.conflict_folder = PathError('conflict_folder')
PathError.conflict_file_ancestor = PathError('conflict_file_ancestor')
PathError.read_only = PathError('read_only')
PathError.shared_folder_disallowed = PathError('shared_folder_disallowed')
PathError.autorename_filed = PathError('autorename_filed')
PathError.other = PathError('other')

CreateFolderArg._path_validator = bv.String()
CreateFolderArg._all_field_names_ = set(['path'])
CreateFolderArg._all_fields_ = [('path', CreateFolderArg._path_validator)]

DeleteArg._path_validator = bv.String()
DeleteArg._all_field_names_ = set(['path'])
DeleteArg._all_fields_ = [('path', DeleteArg._path_validator)]

RelocationArg._from_path_validator = bv.String()
RelocationArg._to_path_validator = bv.String()
...
RelocationError._from_error_validator = bv.Union(PathError)
RelocationError._to_error_validator = bv.Union(PathError)
RelocationError._too_many_files_validator = bv.Void()
RelocationError._quota_error_validator = bv.Void()
RelocationError._other_validator = bv.Void()
RelocationError._tagmap = {
    'from_error': RelocationError._from_error_validator,
    'to_error': RelocationError._to_error_validator,
    'too_many_files': RelocationError._too_many_files_validator,
    'quota_error': RelocationError._quota_error_validator,
    'other': RelocationError._other_validator,
}

RelocationError.too_many_files = RelocationError('too_many_files')
RelocationError.quota_error = RelocationError('quota_error')
...
ThumbnailArg._path_validator = bv.String()
...
ThumbnailError._download_error_validator = bv.Union(DownloadError)
...
ThumbnailError._tagmap = {
    'download_error': ThumbnailError._download_error_validator,
...
PreviewArg._path_validator = bv.String()
...
PreviewError._download_error_validator = bv.Union(DownloadError)
...
PreviewError._tagmap = {
    'download_error': PreviewError._download_error_validator,
