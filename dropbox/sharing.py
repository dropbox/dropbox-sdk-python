...
class RevokeSharedLinkError(object):
    ...
    def __repr__(self):
        return 'RevokeSharedLinkError(%r)' % self._tag
...
class UserInfo(object):
    ...
    :ivar email_verified: Whether the user has verified their email address.
    ...
    def __repr__(self):
        return 'UserInfo(account_id={!r}, display_name={!r}, email={!r}, email_verified={!r}, same_team={!r}, member_id={!r})'.format(
...
class SharedFolderMetadata(object):
    ...
    def __repr__(self):
        return 'SharedFolderMetadata(name={!r}, id={!r}, access_type={!r}, shared_link_policy={!r}, path_lower={!r})'.format(
            self._name_value,
            self._id_value,
            self._access_type_value,
...
class BasicSharedFolderMetadata(SharedFolderMetadata):
    ...
    def __repr__(self):
        return 'BasicSharedFolderMetadata(name={!r}, id={!r}, access_type={!r}, shared_link_policy={!r}, path_lower={!r})'.format(
            self._name_value,
            self._id_value,
            self._access_type_value,
            self._shared_link_policy_value,
...
class FullSharedFolderMetadata(SharedFolderMetadata):
    ...
    def __repr__(self):
        return 'FullSharedFolderMetadata(name={!r}, id={!r}, access_type={!r}, shared_link_policy={!r}, membership={!r}, groups={!r}, path_lower={!r})'.format(
            self._name_value,
            self._id_value,
            self._access_type_value,
            self._shared_link_policy_value,
...
class ListSharedFoldersArgs(object):
    ...
    :ivar include_membership: If include user and group membership information
        in the response.
    :ivar show_unmounted: Determines whether the returned list of shared folders
        will include folders  that the user has left (but may still rejoin).
    """

    __slots__ = [
        '_include_membership_value',
        '_include_membership_present',
        '_show_unmounted_value',
        '_show_unmounted_present',
    ]
    ...
    def __init__(self,
        ...
        self._include_membership_value = None
        self._include_membership_present = False
        self._show_unmounted_value = None
        self._show_unmounted_present = False
        if include_membership is not None:
            self.include_membership = include_membership
        if show_unmounted is not None:
            self.show_unmounted = show_unmounted
    ...
    @property
    def show_unmounted(self):
        """
        Determines whether the returned list of shared folders will include
        folders  that the user has left (but may still rejoin).

        :rtype: bool
        """
        if self._show_unmounted_present:
            return self._show_unmounted_value
        else:
            return False

    @show_unmounted.setter
    def show_unmounted(self, val):
        val = self._show_unmounted_validator.validate(val)
        self._show_unmounted_value = val
        self._show_unmounted_present = True

    @show_unmounted.deleter
    def show_unmounted(self):
        self._show_unmounted_value = None
        self._show_unmounted_present = False

    def __repr__(self):
        return 'ListSharedFoldersArgs(include_membership={!r}, show_unmounted={!r})'.format(
            self._include_membership_value,
            self._show_unmounted_value,
        )

class ListSharedFoldersResult(object):
    """
    Result for list_shared_folders. Unmounted shared folders can be identified
    by the absence of ``SharedFolderMetadata.path_lower``.
...
UserInfo._email_verified_validator = bv.Boolean()
...
UserInfo._all_fields_ = [
    ...
    ('email_verified', UserInfo._email_verified_validator),
...
ListSharedFoldersArgs._show_unmounted_validator = bv.Boolean()
ListSharedFoldersArgs._all_field_names_ = set([
    'include_membership',
    'show_unmounted',
])
ListSharedFoldersArgs._all_fields_ = [
    ('include_membership', ListSharedFoldersArgs._include_membership_validator),
    ('show_unmounted', ListSharedFoldersArgs._show_unmounted_validator),
]
