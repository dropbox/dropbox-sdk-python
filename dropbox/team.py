...
class GroupAccessType(object):
    ...
    :ivar owner: A group owner can rename the group, and add/remove members.
    """
...
class GroupCreateArg(object):
    ...
    :ivar group_external_id: Optional argument. The creator of a team can
...
class GroupSelectorError(object):
    """
...
class GroupsGetInfoError(object):
    """
    This class acts as a tagged union. Only one of the ``is_*`` methods will
    return true. To get the associated value of a tag (if one exists), use the
    corresponding ``get_*`` method.
...
class GroupsGetInfoItem(object):
    """
    This class acts as a tagged union. Only one of the ``is_*`` methods will
    return true. To get the associated value of a tag (if one exists), use the
    corresponding ``get_*`` method.
...
class GroupsListContinueError(object):
    """
    This class acts as a tagged union. Only one of the ``is_*`` methods will
    return true. To get the associated value of a tag (if one exists), use the
    corresponding ``get_*`` method.
...
class MemberProfile(object):
    ...
    def __repr__(self):
        return 'MemberProfile(team_member_id={!r}, email={!r}, email_verified={!r}, status={!r}, name={!r}, external_id={!r})'.format(
            self._team_member_id_value,
            self._email_value,
...
class MembersGetInfoError(object):
    """
    This class acts as a tagged union. Only one of the ``is_*`` methods will
    return true. To get the associated value of a tag (if one exists), use the
    corresponding ``get_*`` method.
...
class MembersGetInfoItem(object):
    """
...
class MembersRemoveArg(MembersDeactivateArg):
    ...
    def __repr__(self):
        return 'MembersRemoveArg(user={!r}, wipe_data={!r}, transfer_dest_id={!r}, transfer_admin_id={!r})'.format(
            self._user_value,
            self._wipe_data_value,
...
class TeamMemberProfile(MemberProfile):
    ...
    def __repr__(self):
        return 'TeamMemberProfile(team_member_id={!r}, email={!r}, email_verified={!r}, status={!r}, name={!r}, groups={!r}, external_id={!r})'.format(
            self._team_member_id_value,
            self._email_value,
            self._email_verified_value,
            self._status_value,
...
class TeamSharingPolicies(object):
    ...
    def __repr__(self):
        return 'TeamSharingPolicies(shared_folder_member_policy={!r}, shared_folder_join_policy={!r})'.format(
...
GroupFullInfo._all_fields_ = GroupSummary._all_fields_ + [
    ('members', GroupFullInfo._members_validator),
    ('created', GroupFullInfo._created_validator),
]

GroupMemberInfo._profile_validator = bv.Struct(MemberProfile)
GroupMemberInfo._access_type_validator = bv.Union(GroupAccessType)
...
GroupMemberSelector._group_validator = bv.Union(GroupSelector)
GroupMemberSelector._user_validator = bv.Union(UserSelectorArg)
...
GroupMembersChangeResult._group_info_validator = bv.Struct(GroupFullInfo)
GroupMembersChangeResult._async_job_id_validator = bv.String(min_length=1)
...
GroupMembersSelector._group_validator = bv.Union(GroupSelector)
GroupMembersSelector._users_validator = bv.Union(UsersSelectorArg)
...
GroupSelector._group_id_validator = bv.String()
...
GroupsGetInfoItem._group_info_validator = bv.Struct(GroupFullInfo)
...
GroupsListResult._groups_validator = bv.List(bv.Struct(GroupSummary))
...
GroupsSelector._group_ids_validator = bv.List(bv.String())
...
MemberAccess._user_validator = bv.Union(UserSelectorArg)
MemberAccess._access_type_validator = bv.Union(GroupAccessType)
...
MemberProfile._team_member_id_validator = bv.String()
...
MemberProfile._status_validator = bv.Union(TeamMemberStatus)
...
MembersAddArg._new_members_validator = bv.List(bv.Struct(MemberAddArg))
...
MembersAddJobStatus._complete_validator = bv.List(bv.Union(MemberAddResult))
...
MembersAddLaunch._complete_validator = bv.List(bv.Union(MemberAddResult))
...
MembersDeactivateArg._user_validator = bv.Union(UserSelectorArg)
...
MembersGetInfoArgs._members_validator = bv.List(bv.Union(UserSelectorArg))
...
MembersGetInfoItem._member_info_validator = bv.Struct(TeamMemberInfo)
...
MembersSetPermissionsArg._user_validator = bv.Union(UserSelectorArg)
MembersSetPermissionsArg._new_role_validator = bv.Union(AdminTier)
...
MembersSetPermissionsResult._team_member_id_validator = bv.String()
MembersSetPermissionsResult._role_validator = bv.Union(AdminTier)
...
MembersUnsuspendArg._user_validator = bv.Union(UserSelectorArg)
...
UserSelectorArg._team_member_id_validator = bv.String()
...
UsersSelectorArg._team_member_ids_validator = bv.List(bv.String())
